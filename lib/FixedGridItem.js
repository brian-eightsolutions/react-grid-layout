...
export default class FixedGridItem extends React.Component<Props, State> {
   ...
   */
  calcPosition(x: number, y: number, w: number, h: number, state: ?Object, margin: [number, number]): Position {
    ...
    if (state && state.dragging) {
      out.top = Math.round(state.dragging.top) + margin[0];
      out.left = Math.round(state.dragging.left) + margin[1];
    }
  ...
  render(): ReactNode {
    ...
    const pos = this.calcPosition(x, y, w, h, this.state, margin);
