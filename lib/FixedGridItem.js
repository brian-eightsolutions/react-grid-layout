...
export default class FixedGridItem extends React.Component<Props, State> {
  ...
  calcXY(top: number, left: number): { x: number, y: number } {
    const { margin, rowHeight, w } = this.props;
    let { colWidth, containerWidth } = this.props;

    // Prevent crashes
    if (!colWidth) colWidth = 0;
    if (!containerWidth) containerWidth = 0;

    // Without Math.round the while statement will enter an infinte loop
    // if the user users their browser to zoom out. Do not remove it.
    let x = Math.round(left > 0 ? left : 0);
  ...
  render(): ReactNode {
    const { x, y, w, h, isDraggable, useCSSTransforms } = this.props;
